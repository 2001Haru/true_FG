"""Canonical fine-grained class prompts for the CoDA adapter.

Aircraft follows the FGVC-Aircraft ``variants.txt`` order. Cars follows the
Stanford Cars ``cars_meta.mat`` class order. CUB prompts are recoverable from
the semantic ImageFolder directory names and therefore are generated at run
time instead of duplicated here.
"""
...
AIRCRAFT_VARIANTS = (
    ...
    "Metroliner", "Model B200", "PA-28", "SR-20", "Saab 2000", "Saab 340",
...
def prompts_for_classes(spec: str, class_folders: list[str]) -> dict[str, str]:
    if spec == "cub":
        prompts = {
            folder: re.sub(r"^\d+\.", "", folder).replace("_", " ")
            for folder in class_folders
        }
    elif spec == "aircraft":
        if len(class_folders) != len(AIRCRAFT_VARIANTS):
            raise ValueError(f"Aircraft class count {len(class_folders)} != 100")
        prompts = dict(zip(class_folders, AIRCRAFT_VARIANTS))
    elif spec == "cars":
        if len(class_folders) != len(STANFORD_CARS_CLASSES):
            raise ValueError(f"Cars class count {len(class_folders)} != 196")
        prompts = dict(zip(class_folders, STANFORD_CARS_CLASSES))
    else:
        raise ValueError(f"Unsupported fine-grained CoDA spec: {spec}")
    if any(not prompt.strip() for prompt in prompts.values()):
        raise ValueError("Every fine-grained class requires a non-empty prompt")
    return prompts
