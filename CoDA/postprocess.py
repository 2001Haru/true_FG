...
def hdbscan_post(args, M, initial_centers,cluster_labels,X, log_file_path, clusterer):
    ...
    if args.spec in ('cub', 'aircraft', 'cars'):
        # Algorithm 2 only requires enough outliers to fill the missing IPC.
        # The released safety multiplier is tuned for large ImageNet classes
        # and can make 24-image fine-grained classes enter an empty split loop.
