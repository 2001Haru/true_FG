...
FINE_GRAINED_SPECS = {'cub', 'aircraft', 'cars'}
...
class ImageFolder(datasets.DatasetFolder):
    def __init__(self,
        ...
        if spec in FINE_GRAINED_SPECS:
            self.classes, self.class_to_idx = self.find_classes(self.root)
            if len(self.classes) != nclass:
                raise ValueError(
                    f"Fine-grained ImageFolder has {len(self.classes)} classes, expected {nclass}: {root}"
                )
        elif nclass < 1000:
