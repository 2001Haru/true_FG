...
DATASET="${1:?usage: locked_ipc_queue.sh DATASET RECOVERY_SEED IPC WAIT_FOR_RESULT}"
...
exec 8>"$LOCK_ROOT/ipc_${DATASET}_r${RECOVERY_SEED}_ipc${IPC}.lock"
flock -n 8 || {
    echo "IPC queue already running: $DATASET r$RECOVERY_SEED IPC$IPC" >&2
    exit 1
}
...
echo "$(date --iso-8601=seconds) locked IPC queue started" > "$STATUS"
...
echo "$(date --iso-8601=seconds) locked IPC queue complete" > "$STATUS"
