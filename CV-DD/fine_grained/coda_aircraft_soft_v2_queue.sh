...
EXP_ROOT="${EXP_ROOT:-/linxi/dataset/FG_CoDA_soft_v2/aircraft_vae_gseed0_v1}"
CODA_ROOT="${CODA_ROOT:-/linxi/dataset/FG_CoDA_standard/v2/vae_space}"
...
GPUS=(${GPUS:-0 1}); EVALS_PER_GPU="${EVALS_PER_GPU:-2}"
...
generation_audit(){ echo "$CODA_ROOT/generation/A_imsize224/n5_mcs2_ms1/ipc$1/gseed0/generation_audit.json"; }
images(){ python - "$(generation_audit "$1")" <<'PY'
import json,sys
print(json.load(open(sys.argv[1]))['synthetic_dir'])
PY
}
fkd(){ echo "$EXP_ROOT/fkd/ipc${1}_bs20_ipc${1}"; }

write_definition(){
python - "$EXP_ROOT" "$CODA_ROOT" "$TEACHER_DIR" "$(git -C "$ROOT_DIR" rev-parse HEAD)" <<'PY'
import json,os,sys
from pathlib import Path
root,coda,teacher=map(Path,sys.argv[1:4]); revision=sys.argv[4]
x={'status':'running','experiment':'aircraft_coda_vae_gseed0_soft_v2','git_revision':revision,
   'dataset':'A_imsize224','feature_space':'vae','generation_seed':0,'ipcs':[1,3,5],
   'student_seeds':[42,43,44],'teacher_seed':42,'new_fkd_sets':3,'new_results':9,
   'student_protocol':'standard_protocol_v2','supervision':'Teacher42 BSSL/FKD, T20 bilateral KL no T2',
   'coda_root':str(coda.resolve()),'teacher_dir':str(teacher.resolve()),
   'expected_result_files':[str((root/f'results/ipc{ipc}_sseed{seed}.json').resolve()) for ipc in (1,3,5) for seed in (42,43,44)],
   'summary':str((root/'summary/coda_soft_v2.json').resolve())}
...
eval_one(){
  ...
  if [[ ! -f "$result" ]]; then
    CUDA_VISIBLE_DEVICES="$gpu" PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python OPENBLAS_NUM_THREADS=1 OMP_NUM_THREADS=1 MKL_NUM_THREADS=1 \
      python -u "$ROOT_DIR/CV-DD/validate/train_fkd.py" --model ResNet18 --ipc "$ipc" \
        --exp-name "coda_vae_g0_soft_v2_ipc${ipc}_s${seed}" --original-data-path "$(images "$ipc")" \
  ...
  python "$ROOT_DIR/CV-DD/fine_grained/record_coda_soft_v2_result.py" --result "$result" \
    --generation-audit "$(generation_audit "$ipc")" --fkd "$(fkd "$ipc")" \
    --teacher "$TEACHER_DIR/ResNet18.pth" --ipc "$ipc" --student-seed "$seed" >> "$log" 2>&1
}
...
main(){
  ...
  if (( ${#pids[@]} )); then wait_all "${pids[@]}"; fi
  python "$ROOT_DIR/CV-DD/fine_grained/summarize_coda_soft_v2.py" --root "$EXP_ROOT" > "$LOG_ROOT/summary.log" 2>&1
