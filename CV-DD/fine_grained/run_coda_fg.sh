...
STAGE="${1:?usage: run_coda_fg.sh STAGE DATASET IPC [STUDENT_SEED]}"
...
ARM_ROOT="$EXP_ROOT/generation/$DATASET/ipc${IPC}"
CONFIG="$ARM_ROOT/generation_config.json"
CODA_OUTPUT="$EXP_ROOT/work/results/$SPEC/Step-25/IPC-$IPC/DF-1.0-GTP-0.9-gamma-0.05/n_15_s_5"
...
LOG_ROOT="$EXP_ROOT/logs/$DATASET/ipc${IPC}"
...
if [[ ! -f "$CONFIG" ]]; then
    python "$ROOT_DIR/CV-DD/fine_grained/record_coda_generation_config.py" \
        ...
        --model-root "$MODEL_ROOT" --output "$CONFIG" > "$LOG_ROOT/config.log"
fi
...
run_coda() {
    (cd "$CODA_DIR" && python -u CoDA_main.py \
        ...
        --IPC "$IPC" --size 224 --seed 0 --n_neighbors 15 \
...
case "$STAGE" in
    ...
    eval-hard)
        ...
        require_file "$AUDIT"
        result="$RESULT_ROOT/$DATASET/ipc${IPC}_gseed0_sseed${STUDENT_SEED}.json"
        ...
        if [[ ! -f "$result" ]]; then
            python -u "$ROOT_DIR/CV-DD/validate/train_fkd.py" \
                ...
                --exp-name "coda_${DATASET}_ipc${IPC}_gseed0_sseed${STUDENT_SEED}" \
        ...
        python "$ROOT_DIR/CV-DD/fine_grained/annotate_coda_result.py" \
            ...
            --generation-config "$CONFIG" --generation-seed 0
