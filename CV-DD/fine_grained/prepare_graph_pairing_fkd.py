...
def guided_permutation(original,classes,neighbors,key):
 ...
 return torch.tensor(result),{'retained_random_receivers':len(retained),'guided_receivers':len(guided),'guided_neighbor_hits':hits,'guided_fallbacks':len(guided)-hits,'guided_receiver_indices':sorted(guided)}
def main():
 ...
 generator=torch.Generator().manual_seed(42);a.destination.mkdir(parents=True,exist_ok=True);stats={'batches':0,'views':0,'retained_random_receivers':0,'guided_receivers':0,'guided_neighbor_hits':0,'guided_fallbacks':0};max_donor_count=0;receiver_digest=hashlib.sha256();pairing_digest=hashlib.sha256()
 with torch.no_grad():
  for epoch in range(400):
   ...
   for batch,offset in enumerate(range(0,300,20)):
    ...
    # The receiver subset and fallback/f deterministic fallback stream are
    # paired across graph arms; only the neighbor graph may differ.
    pairing,s=guided_permutation(original,batch_classes,neighbors,f'fgdd-v2-pairing-v2\0{epoch}\0{batch}')
    guided_indices=s.pop('guided_receiver_indices');receiver_digest.update(bytes(guided_indices));pairing_digest.update(pairing.numpy().tobytes())
 ...
 result={'status':'complete','graph':a.graph,'source_fkd':str(a.source_fkd.resolve()),'destination':str(a.destination.resolve()),'images':str(a.images.resolve()),'teacher':str(a.teacher.resolve()),'teacher_mode':'train BSSL','epochs':400,'batch_size':20,'random_receiver_fraction':0.5,'guided_receiver_fraction':0.5,'pairing':'retain original donors for 10 stable-hash receivers; maximum-cardinality neighbor matching for other 10; stable-hash one-to-one fallback','paired_receiver_namespace':'fgdd-v2-pairing-v2; identical across confusion/randomized graph arms','guided_receiver_schedule_sha256':receiver_digest.hexdigest(),'pairing_schedule_sha256':pairing_digest.hexdigest(),'class_permutation':class_permutation,'random_graph_seed':a.random_graph_seed if a.graph=='randomized' else None,'stats':stats,'guided_hit_rate':stats['guided_neighbor_hits']/stats['guided_receivers'],'max_donor_multiplicity':max_donor_count,'teacher_forward_images':stats['views'],'metadata_replay':['batch members','BSSL microbatch grouping','flip','bbox','CutMix trigger'],'logits':'Teacher prediction on actual mixed view, fp16 raw 100-class'}
