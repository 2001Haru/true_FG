...
def letterbox(image,size=224):
 scale=min(size/image.width,size/image.height);new=(max(1,round(image.width*scale)),max(1,round(image.height*scale)));resized=image.resize(new,Image.Resampling.BILINEAR);canvas=Image.new('RGB',(size,size),FILL);offset=((size-new[0])//2,(size-new[1])//2);canvas.paste(resized,offset);return canvas,new,offset
...
def main():
 ...
 records=[];areas=[];content_native=[];content_low=[];maes=[];cropped=0
 for class_dir in sorted(p for p in a.r0_root.iterdir() if p.is_dir()):
  for current in sorted(class_dir.iterdir()):
   ...
   native_crop=raw.crop(native_box);low_crop=low.crop(low_box);native_out,nsize,noffset=letterbox(native_crop);low_out,lsize,loffset=letterbox(low_crop)
   ...
   recomputed=raw.resize((224,224),Image.Resampling.BILINEAR);difference=ImageChops.difference(recomputed,low);mae=sum(ImageStat.Stat(difference).mean)/3;maes.append(mae)
   records.append({'class':class_dir.name,'image_id':image_id,'raw_path':str(raw_path.resolve()),'r0_path':str(current.resolve()),'raw_size':list(raw.size),'official_bbox_1indexed':[xmin,ymin,xmax,ymax],'margin_fraction_each_side':a.margin,'native_crop_box_0indexed_exclusive':list(native_box),'normalized_window':list(normalized),'lowres_crop_box_224':list(low_box),'window_area_fraction':area,'full_frame':full,'native_letterbox_content_size':list(nsize),'native_letterbox_offset':list(noffset),'lowres_letterbox_content_size':list(lsize),'lowres_letterbox_offset':list(loffset),'r0_vs_raw_direct_resize_pixel_mae':mae})
 if len(records)!=300:raise RuntimeError(f'expected 300 records, got {len(records)}')
 gate=cropped/len(records)>=.5
 manifest={'status':'complete','dataset':'A_imsize224','ipc':3,'source':'exact R0 seed0 300 image identities','raw_images':str(a.raw_images.resolve()),'boxes':str(a.boxes.resolve()),'bbox_format':'official xmin ymin xmax ymax, top-left=(1,1)','r0_preprocessing_audit':'decoded R0 compared with raw RGB direct bilinear warp224; no bbox used','margin_fraction_each_bbox_side':a.margin,'window_rule':'official bbox plus 10% width/height each side, clipped to raw image','output_rule':'crop then aspect-preserving bilinear fit in 224x224; Aircraft-mean RGB letterbox; JPEG quality95 optimize=False','lowres_rule':'same normalized raw window applied to existing R0 224x224 pixels; no access to recovered native detail','images':300,'cropped_images':cropped,'cropped_fraction':cropped/len(records),'training_gate':{'minimum_cropped_fraction':.5,'passed':gate},'window_area_fraction':stats(areas),'native_letterbox_content_fraction':stats(content_native),'lowres_letterbox_content_fraction':stats(content_low),'r0_vs_raw_direct_resize_pixel_mae':stats(maes),'records':records}
