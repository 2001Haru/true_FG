...
def main():
 ...
 for method in ('r0_random','native_subject','lowres_subject'):
  for s in (42,43,44):
   path=a.control_root/f'results/fullframe_cutmix/random_real/source_seed0/ipc3_sseed{s}.json' if method=='r0_random' else a.root/f'results/{method}/ipc3_sseed{s}.json'
 ...
 result={'status':'complete' if len(rows)==9 and not errors else 'failed','dataset':'A_imsize224','ipc':3,'condition':'fullframe+flip+CutMix hard bbox-area CE','new_fkd':0,'new_students':6,'groups':groups,'contrasts':contrasts,'rows':rows,'errors':errors};out=a.root/'summary/aircraft_bbox_hard_fullframe_cutmix.json';out.parent.mkdir(parents=True,exist_ok=True);tmp=out.with_suffix('.json.tmp');tmp.write_text(json.dumps(result,indent=2,sort_keys=True)+'\n');os.replace(tmp,out);print(json.dumps({'status':result['status'],'rows':len(rows),'errors':errors},indent=2));
