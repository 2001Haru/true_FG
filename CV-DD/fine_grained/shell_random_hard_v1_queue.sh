...
LOG_ROOT="$EXP_ROOT/logs_shell_random"
STATUS_PATH="$EXP_ROOT/shell_random_queue_status.json"
MATRIX_PATH="$EXP_ROOT/shell_random_matrix_definition.json"
BASE_SUMMARY="$EXP_ROOT/summary/dino_fivearm_hard_v1.json"
BASE_SUMMARY_SHA256="57ef4ab11464d445ffedfc154edb7d9fd80e84238edd3db0b85739f6c2b29a58"
...
if [[ "$MEMORY_LIMIT_BYTES" =~ ^[0-9]+$ ]] \
    ...
    echo "Unsafe shell-random wave: cgroup memory below $MEMORY_REQUIRED_BYTES" >&2
    exit 2
fi
mkdir -p "$LOG_ROOT" "$EXP_ROOT/summary" "$GLOBAL_LOCK_ROOT"
...
write_definition() {
    ...
    python - "$MATRIX_PATH" "$EXP_ROOT" "$revision" "$protocol_sha" "$GPU_COUNT" \
        "$EVALS_PER_GPU" "$WAVE_SIZE" "$MEMORY_LIMIT_BYTES" "$BASE_SUMMARY_SHA256" <<'PY'
import json, os, sys
from pathlib import Path
path=Path(sys.argv[1]); root=Path(sys.argv[2])
expected=[
 str((root/'results'/d/f'shell_random_rseed{r}'/f'sseed{s}.json').resolve())
 for d in ('CUB_imsize224','A_imsize224','SC_imsize224')
 for r in (0,1,2) for s in (42,43,44)
]
payload={
 'status':'running','experiment':'dino_sixarm_ipc1_shell_random_extension',
 'protocol':'hard_label_v1','git_revision':sys.argv[3],
 'protocol_spec_sha256':sys.argv[4], 'datasets':['CUB_imsize224','A_imsize224','SC_imsize224'],
 'ipc':1,'selection_method':'shell_random','selection_seeds':[0,1,2],
 'student_seeds':[42,43,44],'expected_new_results':27,
 'expected_combined_results':126,'base_fivearm_summary_sha256':sys.argv[9],
 'selection_algorithm':(
  "uniform pseudo-random hash choice within each frozen 70%-95% pure-radial shell"
 ),
 'uses_rival_similarity':False,
 'post_eval_protocol_changed':False,
...
[[ -f "$BASE_SUMMARY" ]] || { echo "missing base summary" >&2; exit 1; }
[[ "$(sha256sum "$BASE_SUMMARY" | awk '{print $1}')" == "$BASE_SUMMARY_SHA256" ]] \
    || { echo "base five-arm summary changed" >&2; exit 1; }
python "$ROOT_DIR/CV-DD/fine_grained/summarize_dino_fourway_hard_v1.py" \
    --experiment-root "$EXP_ROOT" --output "$EXP_ROOT/summary/fivearm_reaudit.json" \
    > "$LOG_ROOT/fivearm_reaudit.log" 2>&1
[[ "$(sha256sum "$EXP_ROOT/summary/fivearm_reaudit.json" | awk '{print $1}')" == "$BASE_SUMMARY_SHA256" ]] \
    || { echo "base 99-result re-audit differs from frozen summary" >&2; exit 1; }
write_definition
write_status running selection
for dataset in "${DATASETS[@]}"; do
    CURRENT_STAGE="selection:$dataset"
    mkdir -p "$LOG_ROOT/$dataset"
    DATA_ROOT="$DATA_ROOT" DINO_SIXARM_EXP_ROOT="$EXP_ROOT" \
        bash "$ROOT_DIR/CV-DD/fine_grained/run_shell_random_hard_v1.sh" \
...
run_eval() {
    local dataset="$1" selection_seed="$2" student_seed="$3" gpu="$4"
    local arm="shell_random_rseed${selection_seed}"
    mkdir -p "$LOG_ROOT/$dataset/$arm"
    CUDA_VISIBLE_DEVICES="$gpu" OPENBLAS_NUM_THREADS=1 DATA_ROOT="$DATA_ROOT" \
        DINO_SIXARM_EXP_ROOT="$EXP_ROOT" \
        bash "$ROOT_DIR/CV-DD/fine_grained/run_shell_random_hard_v1.sh" \
        eval "$dataset" "$selection_seed" "$student_seed" \
...
for dataset in "${DATASETS[@]}"; do
    for selection_seed in "${SELECTION_SEEDS[@]}"; do
        for student_seed in "${STUDENT_SEEDS[@]}"; do
            ...
            run_eval "$dataset" "$selection_seed" "$student_seed" "$gpu" &
            pids+=("$!")
...
python "$ROOT_DIR/CV-DD/fine_grained/summarize_dino_sixarm_hard_v1.py" \
    --experiment-root "$EXP_ROOT" --base-summary "$BASE_SUMMARY" \
    --expected-base-summary-sha256 "$BASE_SUMMARY_SHA256" \
    --output "$EXP_ROOT/summary/dino_sixarm_hard_v1.json" \
