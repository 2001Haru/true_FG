...
def evaluate(checkpoint,root,device,workers):
 dataset=datasets.ImageFolder(root,transforms.Compose([transforms.ToTensor(),transforms.Normalize(MEAN,STD)]));loader=DataLoader(dataset,batch_size=20,shuffle=False,num_workers=workers,pin_memory=True,persistent_workers=workers>0);model=models.resnet18(weights=None);model.fc=nn.Linear(model.fc.in_features,100);payload=torch.load(checkpoint,map_location='cpu',weights_only=False);model.load_state_dict(payload['state_dict'],strict=True);model.eval().to(device);correct=total=0;nll=0.
 with torch.no_grad():
