...
LAMBDA="${1:?usage: run_imagenette_entropy_high_lambda.sh LAMBDA SELECTION_SEED STUDENT_SEED hard|soft1}"
SELECTION_SEED="${2:?missing selection seed}"
STUDENT_SEED="${3:?missing student seed}"
SUPERVISION="${4:?missing supervision}"

[[ "$LAMBDA" =~ ^(8|16|32)$ ]] || { echo "invalid high lambda: $LAMBDA" >&2; exit 2; }
[[ "$SELECTION_SEED" =~ ^(0|1|2)$ ]] || { echo "invalid selection seed" >&2; exit 2; }
[[ "$STUDENT_SEED" =~ ^(42|43|44)$ ]] || { echo "invalid Student seed" >&2; exit 2; }
[[ "$SUPERVISION" == hard || "$SUPERVISION" == soft1 ]] || { echo "supervision must be hard or soft1" >&2; exit 2; }
...
AUDIT="$EXP_ROOT/preflight/high_lambda_extension_audit.json"
GATE="$EXP_ROOT/preflight/high_lambda_extension_gate.json"
MANIFEST="$EXP_ROOT/manifests/lambda_$(printf '%+d' "$LAMBDA")_rseed${SELECTION_SEED}.json"
...
SPEC="$ROOT/class_in_class/imagenette_entropy_high_lambda_protocol.json"
EVAL_EPOCHS=(200 400 600 800 1000 1200 1333 1400 1600 1666 1800 2000)

for path in "$DATA_ROOT/train" "$DATA_ROOT/test" "$TEACHER" "$AUDIT" "$GATE" "$MANIFEST" "$SPEC"; do
    [[ -e "$path" ]] || { echo "missing required input: $path" >&2; exit 1; }
done
python - "$AUDIT" "$GATE" "$TEACHER" <<'PY'
import hashlib,json,sys
from pathlib import Path
audit=Path(sys.argv[1]); gate=Path(sys.argv[2]); teacher=Path(sys.argv[3])
a=json.load(open(audit)); g=json.load(open(gate))
h=hashlib.sha256(teacher.read_bytes()).hexdigest()
ah=hashlib.sha256(audit.read_bytes()).hexdigest()
assert a['status']=='complete_pending_high_lambda_review'
assert g.get('approved') is True and g.get('audit_file')==str(audit.resolve())
assert g.get('audit_sha256')==ah and a.get('teacher_checkpoint_sha256')==h
...
if [[ ! -f "$RESULT" ]]; then
    ...
    python -u "$ROOT/class_in_class/train_imagenette_entropy_selection.py" \
        ...
        --protocol-name imagenette_entropy_high_lambda_v1 --protocol-spec "$SPEC"
fi
