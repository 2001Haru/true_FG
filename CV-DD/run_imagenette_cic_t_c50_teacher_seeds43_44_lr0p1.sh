...
fail(){ echo "ImageNette C50 addon failed: $*" >&2; exit 1; }
wait_pair(){
...
teacher_stage(){
    ...
    local teacher_logs="$MASTER_LOGS/tseed${teacher_seed}/teachers_c50"
    TEACHER_SEED="$teacher_seed" C_VALUES=50 \
...
echo "[A/3] Train and audit C50 Teachers in parallel"
teacher_stage 43 "$GPU0" & teacher43_pid=$!
teacher_stage 44 "$GPU1" & teacher44_pid=$!
wait_pair "$teacher43_pid" "$teacher44_pid" || fail "parallel C50 Teacher stage"

full_stage(){
    local teacher_seed="$1" gpu="$2"
    local teacher_root="$MASTER_ROOT/tseed${teacher_seed}"
    local full_logs="$MASTER_LOGS/tseed${teacher_seed}/full_c50"
    TEACHER_SEED="$teacher_seed" C_VALUES=50 \
...
echo "[B/3] Run both C50 Teacher pipelines in parallel"
full_stage 43 "$GPU0" & full43_pid=$!
full_stage 44 "$GPU1" & full44_pid=$!
wait_pair "$full43_pid" "$full44_pid" || fail "parallel C50 full pipeline"

echo "[C/3] Combine C=1/2/5/10/20/50 across both Teacher seeds"
python "$ROOT/class_in_class/summarize_imagenette_cic_t_teacher_seeds.py" \
    ...
    --output "$MASTER_ROOT/analysis/summary_with_c20_c50.json" \
    > "$MASTER_LOGS/summary_with_c20_c50.log" 2>&1

echo "Complete: $MASTER_ROOT/analysis/summary_with_c20_c50.json"
